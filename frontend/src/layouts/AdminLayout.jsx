...
const NAV = [
  ...
  { to: '/plants',    icon: '🌿', label: 'Plant Catalog' },
  { to: '/locations', icon: '🏢', label: 'Locations'    },
  { to: '/inventory', icon: '📦', label: 'Inventory',   alert: true },
  { to: '/qr',        icon: '🔳', label: 'QR Manager'   },
  { to: '/orders',    icon: '💳', label: 'Orders'       },
  { to: '/refill',    icon: '🔁', label: 'Refill'       },
]
      ...
      <aside style={{ width:230, background:c.forest, position:'fixed', top:0, left:0, height:'100vh', display:'flex', flexDirection:'column', zIndex:100 }}>
          ...
          {NAV.map(n => (
            <NavLink key={n.to} to={n.to} end={n.to === '/'}
              style={({ isActive }) => ({
                ...
                borderRadius:8, cursor:'pointer', fontSize:13.5, textDecoration:'none', marginBottom:2,
        ...
        <div style={{ padding:'14px 18px', borderTop:'1px solid rgba(255,255,255,.08)', display:'flex', alignItems:'center', gap:9 }}>
          ...
          <button onClick={handleLogout} title="Logout"
            style={{ background:'none', border:'none', color:'rgba(255,255,255,.4)', cursor:'pointer', fontSize:16, padding:4 }}>⏻</button>
        </div>
      </aside>
